...
def get_category_and_his_link(content):
    ...
    for item in soup.find_all(class_=["nav nav-list"]):
        for li in item.ul.find_all("li"):
            result.append({"link":get_link_without_page(li.a['href']),"category":li.get_text(strip=True)})
    return result
...
def get_books(content=List[ScrapedLinkAndCategory]):
    ...
    for scraped in content:
        ...
        for item in BeautifulSoup(first_page_content, 'html.parser').find_all("li", class_="current"):
...
def get_book_content(content,category)->List[BookModel]:
    books=[]
    for item in BeautifulSoup(content,'html.parser').find_all("li",class_=["col-xs-6 col-sm-4 col-md-3 col-lg-3"]):
            image=BASE_URL+re.sub(r'^.*(?=/media)', '',item.img['src'])
            title=item.h3.get_text(strip=True)
            price = float(item.find(class_="price_color").get_text(strip=True).replace("£", ""))
            availability=item.find(class_="instock availability").get_text(strip=True)
            rate=item.find(class_="star-rating")['class'][-1]
            books.append({"image":image,"title":title,"price":price,"availability":availability,"rate":rating.get(rate.lower()),"category":category})
    return books
